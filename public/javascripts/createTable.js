...
async function fetchData() {
  try {
    ...
    defaultOption.textContent = "Abrir menu";
    defaultOption.value = "";
...
async function fetchTableInfos(tableName) {
  try {
    ...
    if (data.bool) {
      ...
      const defaultOption = document.createElement("option");
      defaultOption.selected = true;
      defaultOption.textContent = "Abir menu";
      defaultOption.value = "";
      select.appendChild(defaultOption);
      // // Create and append the other options
      data.data.forEach((table) => {
...
async function createForm() {
  ...
  selectElement.addEventListener("change", (event) => {
    if (event.target.value === "PRIMARY KEY AUTOINCREMENT") {
      if(event.target.type === "TEXT") {
        alert("AUTOINCREMENT só é permitido em PKs INTEGER.")
      }
  ...
  insertButton.onclick = () => {
    ...
    if (inputPk === "PRIMARY KEY AUTOINCREMENT" && inputType === "INTEGER") {
      if (pk_count === 2) {
        alert("Uma tabela deve possuir pelo menos uma PK");
        return;
      } else {
        pk_count++;
      }
    ...
    if (inputFK === "yes") {
      if (inputTable === "") {
        alert("Selecione uma tabela para a FK");
        return;
      }
      if (columnTable === "") {
        alert("Selecione uma coluna para a FK");
        return;
      }
    }
  ...
  createButton.onclick = () => {
    ...
    if (rows.length < 2) {
      alert("Adicione pelo menos 2 colunas na tabela");
      return;
    }

    if (!tableName) {
      alert("Adicione um nome a tabela");
      return;
    }
    ...
    const data = Array.from(rows).map((row) => {
      ...
      return {
        ...
        pk: checkPK(cells[1].innerText, cells[2].innerText === "YES"),
