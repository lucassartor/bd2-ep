...
function displayTableData(data) {
  ...
  deleteButton.textContent = "Deletar tabela";
  deleteButton.classList.add("delete_btn");
  deleteButton.onclick = () => {
    if (window.confirm("Tem certeza que quer deletar a tabela?")) {
  ...
  if (data.length > 0) {
    ...
    editTh.scope = "col";
    editTh.textContent = "Editar";
    fixedHeaderRow.appendChild(editTh);
    const deleteTh = document.createElement("th");
    deleteTh.scope = "col";
    deleteTh.textContent = "Deletar";
    ...
    data.forEach((row) => {
      ...
      deleteIcon.onclick = () => {
        if (window.confirm("Tem certeza que quer deletar essa linha?")) {
          fetch("/api/tables/delete", {
            ...
            .catch((error) => {
              console.error("Erro deletando linha:", error);
            });
        }
      };
