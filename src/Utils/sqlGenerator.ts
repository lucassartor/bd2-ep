...
function generateSelectSQL(query: string) {
    // Regular expression to match quoted strings
    const regex = /'([^']*)'/g;
    let params = [];
    let match;

    // Extract all quoted strings (parameters) and replace them with a placeholder
    while ((match = regex.exec(query)) !== null) {
        params.push(match[0]);
        query = query.replace(match[0], `{param${params.length - 1}}`);
    }
    ...
    params.forEach((param, index) => {
        query = query.replace(`{param${index}}`, param);
    });
